...
def update_events_table(event_list):
    ...
    for i in range(1,5):
        ...
        row = Events.query.filter_by(event=e.event) 
        if row is None:
            db.session.add(e)
            db.session.commit()
        else:
...
def get_latest_fighters(UFC_parser,event_list):
    ...
    data_bytes = UFC_parser.get_raw_html(event_list[0]['link'])
...
def populate_bouts_fighters_table(parser,event_data,end=2):
    ...
    for bout in event_data[1:end]:
        ...
        if event is None:
            ...
            event = Events.query.filter_by(event=bout['event']).first()
        try:
            data_bytes = parser.get_raw_html(bout['link'])
...
def main():
    ...
    fight_stats_url = 'http://www.ufcstats.com/statistics/events/completed?page=all'
